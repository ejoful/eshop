...
AppAsset::addCss($this, '@web/css/itemDetail.css');
AppAsset::addCss($this, '@web/css/detail.css');
AppAsset::addCss($this, '@web/css/common.css');
AppAsset::addScript($this, '@web/js/sku.js');
		...
		<div class="tm-clear">
			<div class="tb-property">
				<div class="tb-wrap">
					...
					<div class="tb-key">
						<div class="tb-skin">
							<div class="tb-sku">
							<?php foreach ($sale_prop_arr as $sale_prop) { ?>
								<dl class="tb-prop tm-sale-prop tm-clear <?php if ($sale_prop['is_color_prop']) { echo 'tm-img-prop'; }?>">
									<dt class="tb-metatit"><?= $sale_prop['prop_name'] ?></dt>
									<dd>
									<ul data-property="<?= $sale_prop['prop_name'] ?>" class="tm-clear J_TSaleProp <?php if ($sale_prop['is_color_prop']) { echo 'tb-img'; }?>">
									<?php 
									foreach ($sale_prop['prop_value'] as $prop_value) {
									    $style = "style='background:url(".$prop_value['pic_path'].") center no-repeat'";
									?>
										<li data-value="<?= $prop_value['sid'] ?>" title="<?= $prop_value['value'] ?>"><a href="#" <?= $style ?> role="button" tabindex="0"><span><?= $prop_value['value'] ?></span></a></li>
									<?php } ?>
									</ul>
									</dd>
								</dl>
			...
			<div data-spm="1997427645" class="tb-gallery">
				<div class="tb-booth">
					...
					<span class="zoomIcon">󰄬</span>
					<img id="J_ImgBooth" alt="<?= $model->title ?>" src="<?= $item_img_models[0]['path'] ?>" data-haszoom="700"></a><i class="tm-video-play J_playVideo" style="display:none">󰀓</i><i class="tm-video-stop J_stopVideo" style="display:inline"><b></b><s></s></i>
				</div>
				<div class="tb-thumb-warp">
					<a class="tb-thumb-left"><i></i></a>
					<div class="tb-thumb-content">
						<ul id="J_UlThumb" class="tb-thumb tm-clear">
						<?php foreach ($item_img_models as $item_img) {?>
							<li class=""><a href="#"><img src="<?= $item_img->path ?>" alt="商品预览图"></a></li>
						<?php }?>
						</ul>
					</div>
				...
				<div id="attributes" class="attributes">
					<div class="attributes-list" id="J_AttrList">
						<div class="tm-clear tb-hidden tm_brandAttr" id="J_BrandAttr" style="display:block">
							<div class="name">
								品牌名称：<b class="J_EbrandLogo" target="_blank"><?= $props_str_arr['20000']['prop_value'][0]['value'] ?></b>
							</div>
						</div>
						...
						foreach ($props_str_arr as $key => $value) {
						    $prop_value = implode('&nbsp;', array_column($value['prop_value'], 'value'));
						    if ($key == '20000') {
						        $prop_li_str .= "<li id='J_attrBrandName' title='&nbsp;$prop_value'>品牌:&nbsp;$prop_value</li>";
						    } else {
						        
						        $prop_li_str .= "<li title='&nbsp;$prop_value'>".$value['prop_name'].":&nbsp;$prop_value</li>";
						    }
						}
						echo $prop_li_str;
						?>
...
//销售属性集
var keys = <?php echo $keys_str ?>;
//后台读取结果集
var data = <?php echo $sku_str ?>
</script>
