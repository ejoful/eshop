...
$(function() {
	...
	$('.J_TSaleProp li').each(function() {
		...
		if(selectedObjs.length) {
			...
			$('.J_TSaleProp li').not(selectedObjs).not(self).each(function() {
				...
				if(!SKUResult[testAttrIds.join(';')]) {
					$(this).attr('disabled', 'disabled').removeClass('tb-selected');
				} else {
			...
			//设置属性状态
			$('.J_TSaleProp li').each(function() {
				SKUResult[$(this).attr('data-value')] ? $(this).removeAttr('disabled') : $(this).attr('disabled', 'disabled').removeClass('tb-selected');
			})
		}
	});
});
